  ...
  template: `
      <div *ngFor="let nav of paths" class="space-y-1">
        ...
        <div class="space-y-1" id="sub-menu-1">
          <a *ngFor="let page of nav.pages" [routerLink]="['', nav.key, page.key]" href="#" class="group w-full flex items-center pl-11 pr-2 py-2 text-sm font-medium text-gray-600 rounded-md hover:text-gray-900 hover:bg-gray-50">
            {{ page.label }}
          </a>
        </div>
      </div>
